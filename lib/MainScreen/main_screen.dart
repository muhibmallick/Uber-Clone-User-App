...
import 'package:my_captain/Global/global.dart';
import 'package:my_captain/MainScreen/rate_driver_screen.dart';
import 'package:my_captain/MainScreen/search_places_screen.dart';
import 'package:my_captain/MainScreen/select_nearest_active_driver_screen.dart';
import 'package:my_captain/Models/direction_details_info.dart';
...
import '../main.dart';
import '../secondry wideget/navigation_drawer_widget.dart';
import '../widgets/my_dawer.dart';
...
class _MainScreenState extends State<MainScreen> {
  ...
  // for google map logo for credit padding
  double bottomPaddingOfMap = 0;
// for polyline google map
...
// ask for the permission location even if denied it will popup again
  ...
  locateUserPosition() async {
    // below line gives us the location of current user and assign it variable (with high accuracy)
    Position cPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);
    //same way assign cposition into usercurrentposition variable
    userCurrentPosition = cPosition;
// then conververted into currentposition into latitude and longitude,the way to represent location on google map (! it means it cant be empty)
    LatLng latLngPosition =
        LatLng(userCurrentPosition!.latitude, userCurrentPosition!.longitude);

// below line of code move camera in accordance to your location if u are moving so will your camera will...

    CameraPosition cameraPosition =
        CameraPosition(target: latLngPosition, zoom: 19);
// newcamera position the new point after reaching it, it becomes old, since there is point c
  ...
  void initState() {
    super.initState();
    // we can print the of the current user dynamically from database with the help of this and assistant methods file
...
saveRideRequestInformation()
  ...
  updateArrivalTimeToUserPickupLocation(driverCurrentPositionLatLng) async
  {
    if(requestPositionInfo == true)
    {
      ...
      setState(() {
        driverRideStatus =  "Driver is arriving in " + directionDetailsInfo.duration_text.toString();
      });
  ...
  updateReachingTimeToUserDropOffLocation(driverCurrentPositionLatLng) async
  {
    if(requestPositionInfo == true)
    {
      ...
      setState(() {
        driverRideStatus =  "Going towards Destination in " + directionDetailsInfo.duration_text.toString();
      });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          GoogleMap(
            // for google map logo for credit
